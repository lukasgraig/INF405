...
class Data:
    ...
    def nationwide_query(self):
        ...
        for state in self.query_states.keys():
            ...
            best_mask = state_data[5]
            if float(best_mask[1]) > float(current_best[1]):
                current_best[0] = best_mask[0]
                current_best[1] = best_mask[1] 
            
            worst_mask = state_data[6]
            if float(worst_mask[1]) > float(current_worst[1]):
                current_worst[0] = worst_mask[0]
                current_worst[1] = worst_mask[1]
            
            print(f"{state}\t | \t{state_data[0]:.2f}%\t \t{state_data[1]:.2f}%\t \t{state_data[2]:.2f}%\t \t{state_data[3]:.2f}%\t \t{state_data[4]:.2f}%\nBest county mask wearing rate is {best_mask[0]} with {best_mask[1]:.2f}% and the Worst county mask wearing rate is {worst_mask[0]} with {worst_mask[1]:.2f}%\n")
